    ...
    <SiteLayout>
      <section className="bg-steel-900 text-white py-24">
        <div className="container mx-auto px-6">
          <h1 className="font-display text-5xl md:text-7xl font-bold uppercase mb-4">Contact</h1>
          <div className="w-20 h-2 bg-spark" />
        </div>
      </section>
      <section className="container mx-auto px-6 py-20 grid lg:grid-cols-2 gap-16">
        <div>
          <h2 className="text-2xl font-display font-bold uppercase mb-8">Reach Out</h2>
          <div className="space-y-6">
            <a href={`tel:${c.phone}`} className="flex items-start gap-4 group">
              ...
              <div>
                <div className="text-xs uppercase tracking-widest text-steel-500 mb-1">Phone</div>
            ...
            <a href={`https://wa.me/${c.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="flex items-start gap-4 group">
              ...
              <div>
                <div className="text-xs uppercase tracking-widest text-steel-500 mb-1">WhatsApp</div>
            ...
            <a href={`mailto:${c.email}`} className="flex items-start gap-4 group">
              ...
              <div>
                <div className="text-xs uppercase tracking-widest text-steel-500 mb-1">Email</div>
            ...
            <a href={c.maps_url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-4 group">
              ...
              <div>
                <div className="text-xs uppercase tracking-widest text-steel-500 mb-1">Workshop</div>
        ...
        <form onSubmit={submit} className="bg-steel-50 border border-steel-200 p-8 space-y-4">
          <h2 className="text-2xl font-display font-bold uppercase mb-6">Send a Message</h2>
          <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Name *" className="w-full bg-white border border-steel-200 px-4 py-3" />
          <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email" type="email" className="w-full bg-white border border-steel-200 px-4 py-3" />
          <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="Phone" className="w-full bg-white border border-steel-200 px-4 py-3" />
          <textarea required value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="How can we help? *" rows={5} className="w-full bg-white border border-steel-200 px-4 py-3" />
